...
\end{figure}
%
The average per site, per individual coverage across the 1.7M filtered sites is \Sexpr{round(cov, digits=1)}$\times$ (fig. \ref{Fig:global-coverage-dist}).
%
\begin{figure}[htb]
\centering
...
\begin{figure}[htb]
\centering
<< fst-by-ascertainment-class, out.width='.9\\linewidth', out.height='.4\\textheight', echo=FALSE, cache=TRUE>>=
@
...
\begin{figure}[htbp]
\centering
<< folded-sfs-boot-exh-ery, out.width='.9\\linewidth', echo=FALSE, cache=TRUE, warning=FALSE>>=
@
<< folded-sfs-boot-exh-par, out.width='.9\\linewidth', echo=FALSE, cache=TRUE, warning=FALSE>>=
@
...
\begin{figure}[htb]
\centering
<< S-and-Pi, out.width='.9\\linewidth', echo=FALSE, cache=TRUE, warning=FALSE>>=
@
