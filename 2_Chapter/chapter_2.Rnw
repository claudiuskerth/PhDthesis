...
@
\caption{The global folded \gls{SFS} of \textit{erythropus} and \textit{parallelus} with absolute counts and 95\% \gls{CI} limits from 200 bootstrap resamples of contigs. A standard neutral model was fit to each spectrum for comparison.}
