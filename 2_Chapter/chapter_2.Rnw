...
\end{equation}
This formula gives the neutral expectation of counts in each frequency class (i) in a folded spectrum. I used the R function \texttt{optimize} to find the value of $\theta$ that minimizes the deviation of the above equation from all observed counts $\eta_i$. According to \cite{Fu1995}, p. 192, for large sample sizes (i. e. large $n$) the counts in neutral spectra can be approximated by a Poisson distribution. I therefore added 95\% \gls{CI} limits from Poisson distributions with parameters equal to $E[\eta_i]$ to the fitted neutral spectra.
...
\end{equation}
%
\ldots where $n$ is the number of alleles sampled, i. e. twice the number of sampled individuals for diploid loci. $\eta_i$ is the count in the i--th frequency class of the folded SFS.
...
\todo[inline]{fit secondary contact model}

\todo[inline]{include population size change after split into the model}
...
\todo[inline]{apply Ludovic's correction to 1D and 2D unfolded spectra}
