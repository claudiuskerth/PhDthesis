...
\end{figure}
%
The individual coverage distributions show a median coverage of $1\times$ for all \textit{erythropus} individuals and a median of $0\times$ for most \textit{parallelus} individuals (fig. \ref{Fig:dedup_cov_dist}).
%
\begin{figure}[htb]
\centering
