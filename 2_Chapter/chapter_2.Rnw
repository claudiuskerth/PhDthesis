...
%
%
The PCA in figure \vref{Fig:PCA} has been done on a covariance matrix that has been estimated from genotype likelihoods without SNP and without genotype calling. Additionally, no normalisation of the genotypic covariances by the binomial variance of the \gls{maf} has been applied. 
