...
\usepackage[textsize=tiny, textwidth=3cm, color=Dandelion,colorinlistoftodos,disable]{todonotes} % specify ``disable'' to turn off all notes
...
%\newcommand{\done}{\todo[color=green!60]{#1}}
...
\RequirePackage[labelsep=space,font=footnotesize,labelfont=bf,tableposition=top]{caption}
...
%\usepackage{subfigure} % note, may clash with subcaption package, so if problems then use one or the other
...
\usepackage{ctable}
\usepackage{colortbl}
% set the default font size for all the tables:
